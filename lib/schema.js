...
function defineECSignature () {
  ...
  function encodingLength (obj) {
    ...
    let len = ECPubKey.encodingLength(obj.pubKey)
    length += varint.encodingLength(len)
    length += 1 + len
    if (!defined(obj.sig)) throw new Error("sig is required")
    len = encodings.bytes.encodingLength(obj.sig)
    length += 1 + len
    return length
  }
  ...
  function decode (buf, offset, end) {
    ...
    while (true) {
      ...
      switch (tag) {
        case 1:
        offset += varint.decode.bytes
        obj.pubKey = ECPubKey.decode(buf, offset, offset + varint.decode(buf, offset))
...
function defineECPubKey () {
  ...
  function encodingLength (obj) {
    ...
    let len = encodings.string.encodingLength(obj.curve)
    length += 1 + len
    if (!defined(obj.pub)) throw new Error("pub is required")
    len = encodings.bytes.encodingLength(obj.pub)
    length += 1 + len
    return length
  }
