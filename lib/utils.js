...
exports.sign = function sign (msg, key) {
  ...
  if (key.curve === 'secp256k1') {
    ...
    return Buffer.from(utils.curve(key.curve).sign(msg, key.priv).toDER())
  }
}
...
exports.publicKeyCombine = function (a, b) {
  ...
  if (a.curve === 'secp256k1') {
    val = Buffer.from(secp256k1.publicKeyCombine([a.pub, b.pub], false))
  } else {
    const curve = utils.curve(a.curve)
    a = curve.keyFromPublic(a)
    b = curve.keyFromPublic(b)
    val = Buffer.from(a.pub.add(b.pub).encode('buffer'))
  }
...
exports.encodeSig = function (sig) {
  if (typeof sig.sig === 'string') sig.sig = Buffer.from(sig.sig, 'hex')
...
exports.genECKey = function (curve) {
  ...
  if (curve === 'secp256k1') {
    ...
    pub = Buffer.from(pair.getPublic(false, 'buffer'))
  }
